...
ln -s  "/home/notplus/Desktop/huogui" "$vocPath"
fi
...
if [ -d "$vocPath/annotations_cache" ]; then
rm -rf "$vocPath/annotations_cache"
else
#do nothing
echo "Had not cache"
fi
...
python ./tools/test_demo.py --gpu 0 --def model/voc2007_comp/test.prototxt --weights model/152/HuoGui_iter_60000.caffemodel --cfg model/voc2007_comp/test.yml --imdb voc_2007_test --classes data/VOCdevkit2007/classes_name.txt 2>&1 | tee log/log.txt
